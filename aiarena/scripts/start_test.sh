...
function wait_actor() {
  while true; do
    ...
    if [[ `ps -e f|grep train.py|grep -v grep|wc -l` -eq 0 ]]
    then
        break
    fi
...
do   
    if [[ -e "/aiarena/logs/learner/loss.txt" && ` ls -l /aiarena/logs/learner/loss.txt | awk '{print $5}' ` -ne 0 ]]
    then
        log "time cost: $[` date  +%s` - $start_time]"
        log "---------------------------testing success----------------------------"
        ts_tag=0
        break
    elif [[ `ps -e f|grep train.py|grep -v grep|wc -l` -eq 0 ]]
    then
        log "time cost: $[` date  +%s` - $start_time]"
        log "---------------------------testing fail------------------------------"
        ts_tag=1
        break
    elif [[ `ps -e f|grep entry.py|grep -v grep|wc -l` -eq 0  ]]
    then
        log "time cost: $[` date  +%s` - $start_time]"
        log "---------------------------testing fail------------------------------"
        ts_tag=1
...
if [ $[` date  +%s` - $start_time] -ge $max_test_time ]
then 
    log "time cost: $[` date  +%s` - $start_time]"
    log "---------------------------testing fail------------------------------"
    ts_tag=2
...
for i in $(seq 0 $actor_num); do
    curl --max-time 30 http://$GAMECORE_SERVER_ADDR/v2/stopGame -H 'Content-Type: application/json' -d '{"runtime_id": "localhost-'${i}'"}'
done
