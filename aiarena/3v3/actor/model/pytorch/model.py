...
class Model(nn.Module):
    def __init__(self, Config):
        ...
        self.single_organ_feature_dim = int(self.config.DIM_OF_ORGAN_1_3[0])
        self.singel_monster_feature_dim = int(self.config.DIM_OF_MONSTER_1_20[0])

        self.global_feature_dim = int(np.sum(self.config.DIM_OF_GLOBAL_INFO))
        self.hero_main_feature_dim = int(self.config.DIM_OF_HERO_MAIN[0])
        ...
        self.all_soldier_feature_dim = int(
            np.sum(self.config.DIM_OF_SOLDIER_1_10)
        ) + int(np.sum(self.config.DIM_OF_SOLDIER_11_20))
        self.all_organ_feature_dim = int(np.sum(self.config.DIM_OF_ORGAN_1_3)) + int(
            np.sum(self.config.DIM_OF_ORGAN_4_6)
        )
        self.all_monster_feature_dim = int(np.sum(self.config.DIM_OF_MONSTER_1_20))
    ...
    def forward(self, data_list, inference=True):
        ...
        for hero_index, hero_data in enumerate(data_list):
            ...
            hero_frd = hero_vec_list[0].split(self.config.DIM_OF_HERO_FRD, dim=1)
            _hero_frd = torch.stack(hero_frd, dim=1)
            hero_emy = hero_vec_list[1].split(self.config.DIM_OF_HERO_EMY, dim=1)
            _hero_emy = torch.stack(hero_emy, dim=1)
            hero_main = hero_vec_list[2].split(self.config.DIM_OF_HERO_MAIN, dim=1)[0]
            ...
            organ_1_3 = organ_vec_list[0].split(self.config.DIM_OF_ORGAN_1_3, dim=1)
            _organ_1_3 = torch.stack(organ_1_3, dim=1)
            organ_4_6 = organ_vec_list[1].split(self.config.DIM_OF_ORGAN_4_6, dim=1)
            _organ_4_6 = torch.stack(organ_4_6, dim=1)

            monster_vec_list = split_feature_vec[3]
            monster_1_20 = monster_vec_list.split(
                self.config.DIM_OF_MONSTER_1_20, dim=1
            )
        ...
        for hero_index in range(self.hero_num):
            ...
            all_hero_fc_result_list.append(hero_result_list)

        self.probs_h0 = torch.flatten(
            torch.cat(all_hero_fc_result_list[0], 1), start_dim=1
        )
        self.probs_h1 = torch.flatten(
            torch.cat(all_hero_fc_result_list[1], 1), start_dim=1
        )
        self.probs_h2 = torch.flatten(
            torch.cat(all_hero_fc_result_list[2], 1), start_dim=1
        )
