...
class Model:
    ...
    def _build_infer_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.feature_hero0 = tf.placeholder(
                shape=(self.batch_size, self.hero_data_split_shape[0][0]),
                name="feature_hero0",
                dtype=np.float32,
            )
            hero_data_list.append(self.feature_hero0)
            self.feature_hero1 = tf.placeholder(
                shape=(self.batch_size, self.hero_data_split_shape[1][0]),
                name="feature_hero1",
                dtype=np.float32,
            )
            hero_data_list.append(self.feature_hero1)
            self.feature_hero2 = tf.placeholder(
                shape=(self.batch_size, self.hero_data_split_shape[2][0]),
                name="feature_hero2",
                dtype=np.float32,
            )
    ...
    def _prob(self, pi_logits):
        with tf.variable_scope("prob"):
            return tf.nn.softmax(pi_logits / self.T, axis=-1), tf.argmax(
                pi_logits, axis=-1
            )
    ...
    def build_graph(self, datas, global_step=None):
        ...
        for hero_index in range(self.hero_num):
            this_hero_data_list = self._split_data(
                ...
                self.hero_data_split_shape[hero_index],
            )
    ...
    def _inference(self, each_hero_data_list, only_inference=True):
        ...
        with tf.variable_scope("hero_all_start"):
            conv1_kernel = self._conv_weight_variable(
                shape=[5, 5, self.hero_feature_img_channel[0][0], 18],
            ...
            with tf.variable_scope("hero_%d" % hero_index, reuse=tf.AUTO_REUSE):
                with tf.xla.experimental.jit_scope(self.use_xla):
                    ...
                    split_feature_img, split_feature_vec = tf.split(
                        ...
                        [
                            np.prod(self.hero_seri_vec_split_shape[hero_index][0]),
                            np.prod(self.hero_seri_vec_split_shape[hero_index][1]),
                        ],
                        axis=1,
                    )
                    feature_img_shape = list(
                        self.hero_seri_vec_split_shape[hero_index][0]
                    )
                    feature_img_shape.insert(0, -1)
                    feature_img = tf.reshape(split_feature_img, feature_img_shape)
                    feature_vec_shape = list(
                        self.hero_seri_vec_split_shape[hero_index][1]
                    )
                    ...
                    with tf.variable_scope("hero%d_feature_vec_split" % hero_index):
                        ...
                        soldier_11_20 = tf.split(
                            soldier_vec_list[1],
                            self.Config.DIM_OF_SOLDIER_11_20,
                            axis=1,
                        )
                    ...
                    with tf.variable_scope(
                        ...
                        for index in range(len(hero_frd)):
                            ...
                            fc1_hero_result = tf.nn.relu(
                                ...
                                name="hero%d_fc1_hero_frd_result_%d"
                                % (hero_index, index),
                            )
                            fc1_hero_result_add = tf.nn.relu(
                                (
                                    tf.matmul(
                                        fc1_hero_result, fc1_hero_weight_ten_hero_add
                                    )
                            ...
                            fc2_hero_result = tf.nn.relu(
                                (
                                    tf.matmul(
                                        fc1_hero_result_add, fc2_hero_weight_ten_hero
                                    )
                                    + fc2_hero_bias_ten_hero
                                ),
                                name="hero%d_fc2_hero_frd_result_%d"
                                % (hero_index, index),
                            )
                            fc2_hero_result_add = tf.nn.relu(
                                (
                                    tf.matmul(
                                        fc2_hero_result, fc2_hero_weight_ten_hero_add
                                    )
                            ...
                            fc3_hero_result = tf.add(
                                ...
                                name="hero%d_fc3_hero_frd_result_%d"
                                % (hero_index, index),
                            )
                            # hero_embedding
                            split_0, split_1 = tf.split(
                                fc3_hero_result, [16, 48], axis=1
                            )
                        ...
                        for index in range(len(hero_emy)):
                            ...
                            fc1_hero_result = tf.nn.relu(
                                ...
                                name="hero%d_fc1_hero_emy_result_%d"
                                % (hero_index, index),
                            )
                            fc1_hero_result_add = tf.nn.relu(
                                (
                            ...
                            fc2_hero_result = tf.nn.relu(
                                ...
                                name="hero%d_fc2_hero_emy_result_%d"
                                % (hero_index, index),
                            )
                            fc2_hero_result_add = tf.nn.relu(
                                (
                            ...
                            fc3_hero_result = tf.add(
                                ...
                                name="hero%d_fc3_hero_emy_result_%d"
                                % (hero_index, index),
                            )
                ...
                with tf.xla.experimental.jit_scope(self.use_xla):
                    with tf.variable_scope(
                        ...
                        for index in range(len(hero_main)):
                            ...
                            fc1_hero_result = tf.nn.relu(
                                ...
                                name="hero%d_fc1_hero_main_result_%d"
                                % (hero_index, index),
                            )
                            fc1_hero_result_add = tf.nn.relu(
                                (
                            ...
                            fc2_hero_result = tf.nn.relu(
                                ...
                                name="hero%d_fc2_hero_main_result_%d"
                                % (hero_index, index),
                            )
                            fc2_hero_result_add = tf.nn.relu(
                                (
                            ...
                            fc3_hero_result = tf.add(
                                ...
                                fc3_hero_bias,
                                name="hero%d_fc3_hero_main_result_%d"
                                % (hero_index, index),
                            )
                    ...
                    with tf.variable_scope(
                        ...
                        for index in range(len(monster_1_20)):
                            ...
                            fc1_monster_result = tf.nn.relu(
                                ...
                                name="hero%d_fc1_monster_result_%d"
                                % (hero_index, index),
                            )
                            fc1_monster_result_add = tf.nn.relu(
                                (
                                    tf.matmul(
                                        fc1_monster_result, fc1_monster_weight_add
                                    )
                            ...
                            fc2_monster_result = tf.nn.relu(
                                (
                                    tf.matmul(
                                        fc1_monster_result_add, fc2_monster_weight
                                    )
                                    + fc2_monster_bias
                                ),
                                name="hero%d_fc2_monster_result_%d"
                                % (hero_index, index),
                            )
                            fc3_monster_result = tf.add(
                                tf.matmul(fc2_monster_result, fc3_monster_weight),
                                fc3_monster_bias,
                                name="hero%d_fc3_monster_result_%d"
                                % (hero_index, index),
                            )
                    ...
                    with tf.variable_scope(
                        ...
                        for index in range(len(soldier_1_10)):
                            ...
                            fc1_soldier_result = tf.nn.relu(
                                ...
                                name="hero%d_fc1_soldier_1_result_%d"
                                % (hero_index, index),
                            )
                            fc2_soldier_result = tf.nn.relu(
                                ...
                                name="hero%d_fc2_soldier_1_result_%d"
                                % (hero_index, index),
                            )
                            fc2_soldier_result_add = tf.nn.relu(
                                (
                                    tf.matmul(
                                        fc2_soldier_result, fc2_soldier_weight_add
                                    )
                            ...
                            fc3_soldier_result = tf.add(
                                tf.matmul(
                                    fc2_soldier_result_add, fc3_soldier_weight_frd
                                ),
                                fc3_soldier_bias_frd,
                                name="hero%d_fc3_soldier_1_result_%d"
                                % (hero_index, index),
                            )
                        ...
                        for index in range(len(soldier_11_20)):
                            ...
                            fc1_soldier_result = tf.nn.relu(
                                ...
                                name="hero%d_fc1_soldier_2_result_%d"
                                % (hero_index, index),
                            )
                            fc2_soldier_result = tf.nn.relu(
                                (
                                ...
                                name="hero%d_fc2_soldier_2_result_%d"
                                % (hero_index, index),
                            )
                            fc2_soldier_result_add = tf.nn.relu(
                                (
                            ...
                            fc3_soldier_result = tf.add(
                                tf.matmul(
                                    fc2_soldier_result_add, fc3_soldier_weight_enemy
                                ),
                                fc3_soldier_bias_enemy,
                                name="hero%d_fc3_soldier_2_result_%d"
                                % (hero_index, index),
                            )
                    ...
                    with tf.variable_scope(
                        ...
                        for index in range(len(organ_1_3)):
                            ...
                            fc1_organ_result = tf.nn.relu(
                                ...
                                name="hero%d_fc1_organ_1_result_%d"
                                % (hero_index, index),
                            )
                            fc2_organ_result = tf.nn.relu(
                                ...
                                name="hero%d_fc2_organ_1_result_%d"
                                % (hero_index, index),
                            )
                            ...
                            fc3_organ_result = tf.add(
                                ...
                                fc3_organ_bias_frd,
                                name="hero%d_fc3_organ_1_result_%d"
                                % (hero_index, index),
                            )
                        ...
                        for index in range(len(organ_4_6)):
                            ...
                            fc1_organ_result = tf.nn.relu(
                                ...
                                name="hero%d_fc1_organ_2_result_%d"
                                % (hero_index, index),
                            )
                            fc2_organ_result = tf.nn.relu(
                                (
                                ...
                                name="hero%d_fc2_organ_2_result_%d"
                                % (hero_index, index),
                            )
                            fc2_organ_result_add = tf.nn.relu(
                                (
                            ...
                            fc3_organ_result = tf.add(
                                ...
                                name="hero%d_fc3_organ_2_result_%d"
                                % (hero_index, index),
                            )
                    ...
                    with tf.variable_scope(
                        ...
                        fc_public_result = tf.nn.relu(
                            (
                                tf.matmul(concat_result, fc_public_weight)
                                + fc_public_bias
                            ),
                        ...
                        # individual encoding
                        hero_public_second_result_list.append(
                            this_hero_public_result[1]
                        )
            ...
            with tf.variable_scope("hero_all_end"):
                ...
                for label_index in range(len(self.hero_label_size_list[0])):
                    ...
                    fc2_label_weight = self._fc_weight_variable(
                        shape=[64, self.hero_label_size_list[0][label_index]],
                        name="hero_fc2_label_%d_weight" % (label_index),
                    )
                    fc2_label_bias = self._bias_variable(
                        shape=[self.hero_label_size_list[0][label_index]],
                ...
                fc1_value_bias = self._bias_variable(
                    shape=[64], name="hero_fc1_value_bias"
                )
            ...
            for hero_index in range(len(each_hero_data_list)):
                ...
                with tf.variable_scope("hero%d_out" % hero_index):
                    for label_index in range(
                        len(self.hero_label_size_list[hero_index])
                    ):
                        with tf.variable_scope(
